...
export default function RewardsPage() {
  ...
  const allRewards = groups.flatMap((g) =>
    g.rewards.map((r) => ({ reward: r, projectName: g.project.name, theme: getTheme(g.project.theme_id) }))
  );
    ...
    <main className="max-w-lg mx-auto p-4 pt-5">
      ...
      {allRewards.length === 0 ? (
        ...
        <div className="space-y-5">
          {groups.map((g) => {
            if (g.rewards.length === 0) return null;
            const theme = getTheme(g.project.theme_id);
            return (
              <section key={g.project.id} className="bg-white rounded-2xl border border-line card-shadow overflow-hidden">
                <div className="h-1.5" style={{ background: headerGradient(theme) }} />
                <div className="px-4 py-3">
                  <h2 className="font-bold text-ink text-[14px] mb-3">{g.project.name}</h2>
                  <div className="space-y-2">
                    {g.rewards.map((r, i) => (
                      <button
                        key={i}
                        onClick={() => setSelectedReward({ reward: r, projectName: g.project.name })}
                      ...
                      >
                        <Gift size={18} strokeWidth={2} className="shrink-0" style={{ color: theme.accentDeep }} />
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-bold truncate" style={{ color: theme.accentDeep }}>
                            {r.label}
                          </p>
                          {r.redeemed_at && (
                        ...
                        {r.redeemed_at ? (
                          ...
                          <span className="flex items-center gap-0.5 shrink-0 text-[11px] font-medium" style={{ color: theme.accent }}>
