...
export async function GET(
  ...
  const { data, error } = await supabase
    ...
    .select('issued_at, participant:participants(nickname), tier:project_reward_tiers(label, threshold)')
    .eq('project_id', id)
    .order('issued_at', { ascending: false });

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  type Row = { issued_at: string; participant?: { nickname: string } | null; tier?: { label: string; threshold: number } | null };
