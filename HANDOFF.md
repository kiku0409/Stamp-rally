...
7. **本番のテストデータ汚染**: 2026-06-21 に SQL Editor で掃除済み。
   - `e2e-*` ユーザー・プロジェクトを DELETE（該当なしで成功）。
   - `projects` テーブルには本番2件のみ残存確認済み：
     - `a41b9bf7-...`「きくツアー夏2〇〇〇」（approved・本番用）
     - `2615d2da-...`「既存イベント」（approved・**テスト用のため削除済み**）
   - E2E夏フェス（`807efac3-...`）はテーブルに存在せず（削除済みと推定）。
