...
export default function NicknameForm({ onSubmit, loading }: NicknameFormProps) {
  ...
  const ageValid = age !== '' && !isNaN(ageNum) && ageNum >= 1 && ageNum <= 120;
    ...
    <div className="w-full max-w-sm mx-auto">
      ...
      <div className="bg-white rounded-b-2xl px-5 pb-5 pt-5 card-shadow">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (nickname.trim() && gender && ageValid) {
              onSubmit(nickname.trim(), gender, age);
            }
          }}
          className="space-y-4"
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                max={120}
                required
                className="w-full px-4 py-3.5 rounded-xl border border-line focus:border-accent focus:outline-none text-[15px] font-medium bg-white text-ink"
              />
            </div>
          </div>
