...
### スタンプ帳のボトムナビ（4スロット: ホーム / スタンプ / QR / 引換券）

| スロット | 内容 |
|------|------|
| ホーム | フォトカルーセル・スタンプ進捗・タイムテーブル・会場マップ＋ピン |
| スタンプ | 取得済みスタンプ一覧（イベント名・アイコン・日時） |
| QR（中央の丸ボタン） | QRスキャナーを起動 |
| 引換券 | 獲得した特典チケット一覧（引換用QRコード表示） |

いずれも**選択中プロジェクトのみ**表示（複数参加の場合はヘッダーのチップで切り替え）。ユーザー情報（ニックネーム・性別年齢・復元コード）はボトムナビのタブではなく、ヘッダーのニックネーム/アバターをタップして開く別画面（`/profile`）。
...
- **特典引き換えのリアルタイム通知** — スタッフが特典を引き換え処理すると、来場者がスタンプ帳を開いたままの画面に自動でポップアップ通知が表示される（3秒間隔でポーリング）

### プロジェクトテーマ

スタンプ帳のヘッダー・ボトムナビ・アクセントカラーがアクティブなプロジェクトのテーマカラーに自動切り替わります（8色から選択、シブヤフェス/ストリートライブはダークテーマのカード配色）。ユーザー情報ページのヘッダーも同様のテーマカラーで表示されます。QRスキャンで訪れたプロジェクトが自動でアクティブになります。
...
ユーザー情報画面の「LINEと連携する」からLINEアカウントと紐付けると、機種変更後やLINEアプリ内ブラウザで開いた場合でも「LINEでログイン」だけでスタンプ帳を引き継げます（復元コードの手入力が不要に）。新規ユーザーは「LINEで登録」からも開始でき、LINEの表示名がニックネームにプリフィルされます（性別・年齢は従来通り自前フォームで入力）。連携時にLINEの表示名を保存し、管理者のスタンプ取得者CSVに「LINE連携（済/未）」「LINE表示名」列として出力します（LINE ID・本名・年齢はLINEからは取得できません）。
...
2. **プロジェクト申請** — フェスや連続ライブなどの単位で申請（名称・説明のみ。テーマカラーは承認後にプロジェクト詳細画面から設定）
3. **スーパー管理者が承認** — 承認されるとイベントの作成・QR発行・テーマカラー設定が可能になる
4. **共同編集** — オーナーがメールアドレスで登録済みの他の管理者を追加、または**参加コード**を共有して参加してもらう

> スーパー管理者は全プロジェクト・全イベントを閲覧し、申請の承認/却下のみ行います（他プロジェクトの編集は不可）。
> 「メール招待」は招待メールを送信するものではなく、**そのメールアドレスで既にセルフ登録済みのユーザー**をその場でメンバーに追加する機能。未登録のメールアドレスは追加できない。
...
- **マップ上のピン位置設定** — プロジェクトに会場マップを登録済みなら、マップ画像をクリックするだけでそのイベントのピン位置（X%/Y%）とラベルを設定できる。ラベル・X%/Y%は数値入力でも直接指定可能（会場マップ未登録でも数値入力欄は表示される）
...
- スタッフは `/admin/redeem` で来場者の特典QRをスキャン → 名前と特典を確認 → 「引き換えする」→「確定（引き換える）」の2段階確認で使用済み化
...
### 来場者
- QRスキャンによるスタンプ獲得（ログイン不要・ニックネームのみ、性別・年齢は任意）
- スタンプ帳（ボトムナビはホーム / スタンプ / QR / 引換券の4スロット、ユーザー情報はヘッダーから）
- プロジェクト別テーマカラー（ヘッダー・ボトムナビが自動切り替え、8色）
...
- テーマカラー選択（承認後、プロジェクト詳細画面でスタンプ帳のUIカラーを変更）
...
- 複数管理者での共同編集（登録済みメールアドレスの追加・参加コード共有）
- QRコード発行・ダウンロード
- 特典段階の設定・編集
- 特典引き換え（スキャン方式、二重防止）
- 取得者一覧・CSV書き出し（性別・年齢・LINE連携有無・LINE表示名を含む）
...
>
> **スキーマ運用ルール:** スキーマを変更するときは `supabase/migrations/` にマイグレーションファイルを追加し、**必ず `schema.sql` にも同じ変更を反映する**（片方だけ更新すると本番DBと `schema.sql` が乖離する。実例: SCHEMA-001）。
...
`tests/` に Playwright の E2E テスト 15 本を用意している（管理者認証・スタンプ取得・来場者タブ/プロジェクト切替・マップピン・プロフィール・プロジェクト承認・スタンプCSV・特典引換フロー）。

### 必要な環境変数（`.env.local`）

アプリ用の 3 変数に加えて、テスト用の管理者アカウント情報が必要:

```env
NEXT_PUBLIC_SUPABASE_URL=...
NEXT_PUBLIC_SUPABASE_ANON_KEY=...
SUPABASE_SERVICE_ROLE_KEY=...
TEST_ADMIN_EMAIL=...        # 登録済みの管理者アカウント
TEST_ADMIN_PASSWORD=...
```
...
```bash
# 初回のみ: ブラウザをインストール（Linux は OS ライブラリも必要 → npx playwright install-deps 相当）
npx playwright install chromium

# 実行（localhost:3001 に dev サーバを自動起動して直列実行）
npx playwright test
```

### テストデータ

テストは `.env.local` の Supabase に対して service role で直接シードする。すべてのテストデータには `__playwright_test__` タグが付き、テスト終了時に自動クリーンアップされる。
...
├── theme_id        TEXT NOT NULL DEFAULT 'teal'  -- テーマカラーID
...
├── line_user_id  TEXT             -- LINEログインのuserId（連携済みなら非NULL・UNIQUE）
├── line_display_name TEXT         -- LINE表示名（連携/復元のたびに更新）
